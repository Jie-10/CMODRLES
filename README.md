...
| 收敛辅助种群 | AP | 否 | 基于权重向量的 Tchebycheff 替换 | 从不同目标方向推动无约束收敛 |
...
若遍历全部参考向量后仍少于 $N$ 个解，则从剩余候选解中按 SPEA2-CDP 适应度升序补足。

这使 DP 同时具备方向覆盖和约束感知能力。需要注意，当前代码在计算 DP 夹角前**没有进行理想点平移或目标归一化**。
